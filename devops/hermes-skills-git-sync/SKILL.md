...
- **`-L` flag is required on ALL rsync calls — including dry-runs and diff checks.** Skills in `~/.hermes/skills/` may be symlinks to `~/.agents/skills/` (common for hub-installed or shared skills). Without `-L`, rsync silently skips symlinks with "skipping non-regular file" and reports zero changes, producing a **false negative** on diff checks. A dry-run `rsync -rcn` (no `-L`) will claim "nothing to sync" even when symlinked skills have real content changes. Always use `-L` on every rsync invocation — the sync command, the dry-run preview, and any post-sync `diff` verification. Example failure: `rsync -rcn` → 0 changes reported; `git status` → 10 files actually differ.
