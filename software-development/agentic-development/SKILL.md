...
metadata:
  hermes:
    ...
    related_skills: [hiclaw, codebase-communication, ai-agent-research, agent-loop-architecture]
...
当用户要审查或设计"自主循环执行引擎"、"autoloop"、"self-correcting agent"时，
使用 `agent-loop-architecture` skill — 它覆盖多 tool call/turn、校验分层、
上下文压缩、反思机制等架构决策。
