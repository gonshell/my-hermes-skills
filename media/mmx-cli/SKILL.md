...
mmx CLI 安装在 `~/.hermes/node/bin/mmx`，依赖 `~/.hermes/node/bin/node`。

**不在这两个路径时需要全路径调用：**

```bash
# 全路径调用（PATH 不包含 ~/.hermes/node/bin 时使用）
/Users/xiesg/.hermes/node/bin/mmx text chat --message "Hi" --output json
/Users/xiesg/.hermes/node/bin/node --version
```

认证信息在 `~/.mmx/credentials.json`。
...
| `--channels <n>` | number | Channels (default: 1) |
...
## text chat output structure

The JSON response is an **array of message objects**, not a single object with `.content`:
...
Correct extraction:
```bash
mmx text chat --message "Hi" --output json | jq '.[] | select(.type=="text") | .text'
# → "Hello! How can I help you today?"

# jq '.content' returns empty — there is no '.content' field in the response
```

## image generate — resolution limits

`image-01` model enforces strict pixel bounds:
- Width: 512–2048, must be multiple of 8
- Height: 512–2048, must be multiple of 8
- Both must be set together, or use `--aspect-ratio`

**"8K" in a prompt does not produce 8K resolution** — the API ceiling is 2048px. The aesthetic quality claim is honored but the physical resolution is capped. Do not promise users actual 8K output via mmx image.

## Piping Patterns

```bash
# Correct: extract text from the array response
mmx text chat --message "Hi" --output json | jq '.[] | select(.type=="text") | .text'
...
### execute_code PATH 限制

`execute_code`（Hermes Agent 沙盒子进程）的 PATH 是极简版：

```
/usr/bin:/bin:/usr/sbin:/sbin
```

**不包含** `~/.hermes/node/bin`。在 `execute_code` 中调用 mmx 必须用全路径：

```bash
/Users/xiesg/.hermes/node/bin/mmx text chat --message "Hi" --output json
```

直接调用 `mmx`（不带路径）会报 `command not found`。

### Hermès Chat vs Terminal session

- **Hermes Chat**：mmx 走 MCP 工具（`mcp_minimax_*`），有独立 credential pool，不需要配置 PATH
- **Terminal session**：`mmx` 命令依赖 PATH 中有 `~/.hermes/node/bin`；缺失时用全路径
- **execute_code**：沙盒环境 PATH 极简，必须用全路径

### npm 不在 PATH

npm 安装在 `~/.hermes/node/bin/npm`。如果需要手动安装全局包：

```bash
~/.hermes/node/bin/npm install -g mmx-cli
```