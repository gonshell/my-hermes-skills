...
**⚠️ 大段 XML/JSON 内容不要直接内联命令行**：直接 `echo '...' | lark-cli ... --content -` 或 `... --content '长XML'` 会因 shell 解析 `<>`/`&` 等字符而失败。正确做法：

```bash
# 步骤 1：把内容写入文件（用绝对路径写到 /tmp 或当前目录）
cat > /tmp/doc_content.xml << 'EOF'
<h1>章节标题</h1>
<ol><li>...</li></ol>
EOF

# 步骤 2：复制到当前目录（lark-cli 要求相对路径）
cp /tmp/doc_content.xml ./lark_content.xml

# 步骤 3：用 @./filename 引用（必须是相对路径，不能是 /tmp/xxx）
lark-cli docs +update --api-version v2 --doc "<doc_id>" --command append \
  --content @./lark_content.xml
```

> **`@filepath` 必须是相对路径**，不能是 `/tmp/xxx` 这类绝对路径，否则报错：`--file must be a relative path within the current directory`。
