...
**Key insights from a 270-page math textbook session**:
- Write the OCR program in **Swift** — the `[.language: "zh-Hans"]` parameter set in code is more
  reliable for CJK languages than argv-based language passing in ObjC.
- For math textbooks (dense small text, superscripts/subscripts), render at **300 DPI** and/or
  3× page size (≈1786×2526 px at 150 DPI) to capture fine detail.
- Compile the Swift binary once to a permanent path (e.g. `/tmp/vision_ocr_main_bin`) and reuse
  across sessions.
- For 200+ page jobs: divide into 4 parallel bash jobs to stay under per-command timeout.
...
- **macOS Vision OCR Swift workflow**: see `references/macos-vision-ocr-swift-workflow.md` — working
  Swift script, tested on a 270-page math textbook. Includes parallel batch runner and 300 DPI
  rendering tips for math content.
