...
## 13. `docs +update --new-title` 无法修改飞书文档 UI 标题（已实测，2026-05-30）

**问题**：通过 `lark-cli docs +update --new-title "xxx"` 尝试修改飞书文档的 UI 标题。

**实测结果**：
- v1 API：`--new-title` 参数传入不报错，写入返回 `success`，revision 增加，但 `+fetch` 返回 `title: null`，文档 UI 标题不变
- v2 API：`+update` 根本不支持 `--new-title` 参数，报 `unknown flag: --new-title`
- 尝试裸调 `PATCH /open-apis/docx/v1/documents/{token}` 报 `1770001 invalid param`

**结论**：飞书文档 UI 标题无法通过 lark-cli API 修改。

**用户偏好**：当需要修改文档标题时，用户倾向**删除重建**（而非手动在飞书 UI 操作）：
```bash
# 删除旧文档
lark-cli drive +delete --file-token "<token>" --type docx --yes

# 新建文档（--new-title 改为 --title）
lark-cli docs +create --api-version v2 --title "新标题" --content @./file.xml --doc-format xml
```

> 如果用户坚持要用 API 修改标题而非重建，告知这是飞书 API 的限制，无法通过 lark-cli 实现。

**来源**：2026-05-29，用户要求修改 YouTube AI热门、Bilibili全站热门 文档标题时验证。

---

## 14. XML 内容禁止包含 `<title>` 标签（避免飞书解析为默认标题）

**问题**：写入 XML 内容时包含 `<title>xxx</title>` 标签，飞书会将其解析为文档标题（`title: "xxx"`），覆盖手动设置的标题。每次 overwrite 后 `+fetch` 返回 `title: "Untitled"`（从 `<title>Untitled</title>` 解析），说明飞书系统默认用 XML 中的 `<title>` 作为文档标题字段。

**场景**：cron 定时任务写入每日热门视频到飞书文档，如果 XML 内容含 `<title>` 标签，会导致：
1. 手动设置的标题被覆盖
2. `degrade_code=1011`（内容无变化）问题加剧（每次 revision 递增但标题覆盖导致 UI 异常）

**规则**：
- **禁止**在 `append` / `overwrite` 的 `--content` XML 中使用 `<title>` 标签
- XML 内容只写正文块：`<h1>`、`<p>`、`<callout>`、`<table>` 等
- 时间戳注释放在 `<!-- 任务名 | YYYY-MM-DD HH:MM:SS -->` 而不是 `<title>`
...
**错误 XML 结构示例**：
```xml
<title>YouTube AI热门视频</title>   <!-- ❌ 禁止 -->
<h1>YouTube AI热门视频</h1>
...
```

**来源**：2026-05-29，cron 任务文档标题修复验证中发现。
