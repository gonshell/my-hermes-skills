...
**⚠️ 注意**：`references/bilibili-ai-trending-feishu-workflow.md` 已废弃，内容已整合至 `references/bilibili_processor.md`。

### 完整 cron job 流程

```
1. browser_navigate → order=pubdate + AI 人工智能（第一页）
2. browser_console 提取 h3 meta（标题+作者+日期）和 a[href*="/video/BV"] raw（播放量+时长）
3. 滚动 + 再提取
4. browser_navigate → order=pubdate + AI 人工智能 大模型 深度学习（第二页）
5. 同样提取两次
6. write_file: Python 脚本（含中文数据）写入 .py 文件
7. terminal: python script.py 执行合并 + calc_score 排序 + 分组
8. write_file: XML 内容写入本地文件
9. terminal: lark-cli docs +update overwrite 写入飞书文档
```

### lark-cli 命令要点（2026-05 实测纠错）
| 操作 | 正确命令 | 错误写法 |
|------|---------|---------|
| 读取文档 | `+fetch --api-version v2 --doc "token"` | `--doc-token` |
| 写入文档 | `+update --api-version v2 --doc "token" --command overwrite --content @./filename` | `--mode overwrite` / `@/tmp/xxx` |
| 文件路径 | **相对路径** `@./filename` | 绝对路径 `@/tmp/xxx` |

**常见错误**：`--content @/tmp/output.xml` → lark-cli 报错 `--file must be a relative path within the current directory`。

### ⚠️ 飞书写入模式：overwrite 是全量替换

`+update --command overwrite` 会**替换整个文档**，不会追加内容。

如果需要保留历史内容：先 `+fetch` 读取旧内容，解析并合并，再写入。但更实用的 cron 模式是**直接全量覆盖**，因为：
- 搜索排序本身已是"最新"内容
- 文档中只保留最近 7 天数据，不需要历史堆积
- 避免复杂解析引入错误

### ⚠️ 飞书文档标题保护：XML 绝不能包含 `<title>` 标签

> **详见 `lark-doc-pitfalls` skill（`lark/lark-doc-pitfalls/SKILL.md`）第13-14条。** 以下为摘要：

**问题**：XML 内容里的 `<title>` 标签会被飞书解析为文档标题，导致手动设置的标题被覆盖。每次 overwrite 后文档标题变成 `<title>` 里的字符串。

**✅ 正确做法**：
1. XML 内容**不写 `<title>` 标签**，只包含正文段落（`<h1>`、`<p>`、`<callout>` 等）
2. 用 XML 注释 `<!-- 任务名 | YYYY-MM-DD HH:MM:SS -->` 标记时间戳版本（确保每次内容不同，避免 `degrade_code=1011`）
3. 文档标题在飞书界面手动设置一次即可

**示例（正确）**：
```xml
<!-- Bilibili AI热门视频 | 2026-05-29 21:00:00 -->
<h1>Bilibili AI热门视频</h1>
<p>📺 一、最热门长视频 TOP 15（3天内）</p>
...
```

**示例（错误）**：
```xml
<title>Bilibili AI热门视频 (2026-05-29 21:02:55)</title>   <!-- ❌ 禁止 -->
<h1>Bilibili AI热门视频</h1>
...

### ⚠️ `degrade_code=1011` = 内容未真正更新（2026-05-29 新发现）

**含义**：`"Instruction produced no document changes. The instruction content may be identical to the current document, or the format is unexpected"`

**触发条件**：对同一份内容再次执行 `overwrite`，飞书 API 认为内容无变化，返回 `partial_success` + `revision_id` 变化（看似成功），但实际上文档内容并未刷新。

**现象**：
- revision_id 从 5→6 变化（看起来写成功了）
- 但文档内容和之前完全一样（没有任何更新）
- 后续 cron run 写入的也是相同内容（相同视频列表）

**判断真正写入成功**：不能只看 revision_id，必须 `+fetch` 读取文档内容并比对是否有实际新增内容。

**根本原因**：cron job 每次抓取的 B站热门视频排序相似，导致 output.xml 内容和上次几乎相同 → 飞书认为是重复写入。

**解决方案**：在写入后主动用 `+fetch` 验证，或在写入前记录当前文档内容的长度/时间戳，对比确认有变化再写入。

### Python 脚本执行：中文文本处理（重要！）

当 Python 代码中包含中文文本时，**不要用 `execute_code`**，因为 shell 字符串转义会损坏中文引号（`"` / `"`）导致 SyntaxError。

**✅ 正确做法**：用 `write_file` 将脚本写入 `.py` 文件，再用 `terminal python script.py` 执行：

```bash
# 写入脚本（write_file 正确处理中文编码，无引号转义问题）
write_file content="<含中文的 Python 代码>" path="./bilibili_processor.py"

# 执行脚本
terminal command="python bilibili_processor.py"
```

**❌ 错误做法**：在 `execute_code` 多行字符串中嵌入中文文本 → SyntaxError
**原因**：`execute_code` 通过 shell 执行，shell 会尝试解析/转义中文引号，导致字符串边界破坏
**解决方案**：先 `cp /tmp/output.xml ./`，再用 `@./output.xml`。
...
### 飞书写入成功判断
- 返回 `revision_id` 变化（如 4→5）= 写入成功
- 返回 `"result": "partial_success"` + warning `degrade_code=1011` = 内容可能未变但调用结构正确（通常无害）
...
`references/bilibili_processor.md` — 完整数据处理参考实现（2026-05-28验证），包含：
- `parse_views_and_duration()`: 处理粘连的播放量+时长字段（`"2901403:53:11"` → views=29014, dur="3:53:11"）
...
- 完整 cron job 推荐流程 + 飞书写入路径纠错

`references/feishu-write-pitfalls-20260529.md` — 飞书写入的两个新发现（P1: `<title>` 标签覆盖文档标题；P2: `degrade_code=1011` = 内容未真正更新）+ cron job checklist
`references/feishu-cron-pitfalls-20260529b.md` — P3/P4：`--new-title` 参数无效、7天过滤逻辑缺陷
