...
### Step 1b — Extract chapter timestamps from structured description

YouTube's structured description container often has chapter data that's cleaner than parsing the raw description text. Use browser console:

```javascript
// Get the structured description (chapters are here, not in #description-inline-expander)
const structuredDesc = document.querySelector('#structured-description');
const itemsCont = document.querySelector('#items');  // full expanded description

// Get full description text (with chapter timestamps like "00:00 Topic Name")
const fullText = itemsCont?.innerText || '';
...
**Key DOM elements** (as of 2026-06):
- `#structured-description` — contains chapter list with timestamps (structured, not raw text)
- `#description-inline-expander` — truncated description, needs "show more" click
- `#items` inside `ytd-structured-description-content-renderer` — full expanded description text with all chapter lines
- The "...更多" (show more) button is `ref=e34` or similar; click it to expand before extracting

### Step 1c — Cross-reference official event pages (for conference/keynote videos)

When the video is a **conference keynote, product launch, or event talk**, the official event website often contains structured information that significantly enriches the summary:

- **Event agenda/schedule** — session titles, times, speakers
- **Speaker bios** — names, titles, companies, expertise areas
- **Key themes/tracks** — the official categorization of topics
- **Announcements** — press releases, blog posts linked from the event page

**How to find it:** Look in the video description for links to the event page (e.g., `nvidia.com/en-tw/gtc/taipei/`). Extract via `browser_navigate` + `browser_console`.

**What to extract:**
1. Event dates, location, tagline
2. Main tracks/themes (usually as section headings)
3. Speaker list with affiliations
4. Recommended/key sessions with descriptions
5. Any "read the news" or announcement links

**Merge strategy:** Use event page data to provide context that the video metadata alone cannot — specifically the **ecosystem of participants** (which companies, which roles) and the **thematic structure** (what topics the event organized around). The video chapter timestamps give you the *what*; the event page gives you the *who* and *why this matters*.
