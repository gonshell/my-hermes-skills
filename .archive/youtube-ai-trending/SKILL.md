...
## P5：cronjob 环境下 expanduser 路径偏移（2026-05-30）

`execute_code` 中 `os.path.expanduser("~/.hermes/cron/output/")` 在 cronjob Python 进程中展开为 `/Users/xiesg/.hermes/home/.hermes/cron/output/`，比预期多了 `.hermes/home` 前缀层。

**解法**：硬编码绝对路径：
```python
output_dir = "/Users/xiesg/.hermes/home/.hermes/cron/output/"
```

---

## P6：lark-cli --content 只接受 CWD 相对路径（2026-05-30）

`--content @/absolute/path` 和 `--content @~/path` 均报错或报 not found。必须从 HERMES_HOME（`/Users/xiesg/`）用相对路径。

**最简解法**：
```python
# execute_code：将合并内容写入 HERMES_HOME 根目录
temp_path = "/Users/xiesg/merged_youtube-ai.xml"
with open(temp_path, "w") as f: f.write(merged)
```
```bash
cd /Users/xiesg && lark-cli docs +update --content @merged_youtube-ai.xml ...
```
...
**当前实现逻辑（2026-05 改造）**：
1. 读取文档现有内容（`lark-cli docs +fetch`）
...
# ⚠️ 错误：doc +get-content --doc-token（v1 命令，已废弃）
```
...
# ⚠️ 错误：doc +overwrite --doc-token（v1 命令，已废弃）
# ⚠️ 错误：--mode overwrite（v1 参数，v2 必须用 --command）
```
